...
const historyReturn = useRefHistory(formConfig as any, {
  deep: true, // 当使用对象或数组时，由于更改它们的属性不会更改引用，因此不会触发提交。要跟踪属性更改，您需要传递 deep: true 。它将为每个历史记录创建克隆
  capacity: 20, // 保留的最大历史记录量
  /**
   * 反序列化, 它将序列化后的数据解析成对象
   * @param val
   */
